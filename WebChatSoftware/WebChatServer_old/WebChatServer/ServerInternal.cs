    ...
    class ServerInternal
    {
        ...
        public void Run()
        {
            try
            {
                ...
                if (verbose) { Console.WriteLine("{0} {1} FLOPS server listening on: {2}:{3}.", Program.globalAccumulator, this, localIP, listeningPort); }
                tcpListener.Start();
                var mrs = new Thread(new ThreadStart(MessageReceiveService));
                mrs.Start();
                ...
                try
                {
                    ...
                    try
                    {
                        if (Regex.IsMatch(data, "(^GET /ws)"))
                        {
                            ...
                            var cc = new Client(connection, dataLink);
        ...
        private void MessageReceiveService()
        {
            while (true)
            {
                foreach(var cli in connectedClients)
                {
                    ...
                    {
                        Message rm = null;
                        rm = cli.Value.Receive();
                        if (rm != null)
                        {
                            messageCache.TryAdd(messageCount.getValue(), rm);
                            messages.Add(rm);
                    ...
                    {
                        Client t;
                        connectedClients.TryRemove(cli.Key, out t);
                    }
                }
            }
        }
