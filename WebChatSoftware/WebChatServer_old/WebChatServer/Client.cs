...
namespace WebChatServer
{
    class Client
    {
        BlockingCollection<Message> messageStore = new BlockingCollection<Message>(10);
        private TcpClient clientConnection;
        private NetworkStream dataStream;
        private bool verbose = false;
        private bool messages = false;

        public Client(TcpClient clientConnection, NetworkStream dataStream, bool verbose = true)
        {
            ...
            using (var br = new BinaryReader(dataStream))
            {
                while (true)
                {
                    ...
                    if (d.Length > 2)
                    {
                        ...
                        messageStore.Add(new Message(d[0], d[1], d[2]));
                        messages = true;
                    }
                    else
                    {
