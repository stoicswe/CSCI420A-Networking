...
namespace RayTracer_Client
{
    ...
    class MainClass
    {
            ...
            while (true)
            {
                ...
                if (recievedData.Length > 0)
                {
                    ...
                    if (received[1].Equals("renderdata"))
                    {
                        ...
                        //Console.WriteLine($"{xymax - xymin}, {cls.Count}");

                        for (int dxy = xymin; dxy < xymax-1; dxy++)// xymax-1
                        {
                            int x = dxy % 1200;
        ...
        public static int[] getOpenJob(double percent){
