...
namespace RayTracer {
	...
	internal Color RenderPixel(Scene scene, int x, int y)
	{
            Color color = TraceRay(new Ray() { Start = scene.Camera.Pos, Dir = GetPoint(x, y, scene.Camera) }, scene, 0);
	    return color;
	}
        ...
        internal static Scene ReadScene(String fileName)
        {
            ...
            return new Scene() { Things = os.ToArray(), Lights = ls.ToArray(), Camera = c };
        }
    ...
    {
        public const int width = 600*2;
        public const int height = 600*2;    
	...
	public static double[] RenderPixel(int x, int y)
	{
	    Color color = r.RenderPixel(s, x, y);
        return new double[]{color.R, color.G, color.B};
	}


        static void WritePPM(String fileName, Color[,] bitmap)
        {
